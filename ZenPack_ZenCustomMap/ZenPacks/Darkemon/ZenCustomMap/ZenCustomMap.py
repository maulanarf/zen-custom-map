...
class ZenCustomMapData(BrowserView):
    ...
    def _getDevicesEvents(self, deviceList):
        ...
        result = {}
        try:
            xml = etree.fromstring(deviceList)
        except:
            _log.error(str(deviceList))

        for d in xml:
            events = self.context.zport.dmd.ZenEventManager.getEventList(where="ipAddress='"+d.text+"'")
            maxSeverity = 0
            for e in events:
            ...
            if maxSeverity != 0:
                result[d.get("id")] = maxSeverity

        root = etree.Element("devices_events")
        for k,v in result.items():
            id = etree.Element("id")
            id.text = str(k)
            id.set("severity", str(v))
            root.append(id)
        return etree.tostring(root)
    ...
    def _defaultMap(self, mapId):
        ...
        child = etree.Element("id") # Add 'map' child.
        ...
        child = etree.Element("scale") # Add 'scale' child.
        child.text = "1"
        root.append(child)
